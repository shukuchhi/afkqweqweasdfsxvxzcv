...
def run(serial=None):
    ...
    # 4. По скриншоту collect5_2.png нажимаем на кнопку
    attempt = 0
    while attempt < max_attempts:
        screenshot_path = take_screenshot(device)
    ...
    time.sleep(2)
    fourth_coords = (333, 189)  # Замени на свои координаты
    print(f"Tapping fourth button at {fourth_coords}")
    tap(device, fourth_coords[0], fourth_coords[1])

    # 5. Через 0.5 секунды нажимаем на кнопку
    print("Waiting for 0.5 seconds...")
    time.sleep(3)
    fifth_coords = (416, 860)  # Замени на свои координаты
    print(f"Tapping fifth button at {fifth_coords}")
    tap(device, fifth_coords[0], fifth_coords[1])

    # 7. Через 0.5 секунды нажимаем на кнопку
    print("Waiting for 0.5 seconds...")
    time.sleep(5)
    seventh_coords = (169, 52)  # Замени на свои координаты
    ...
    print("Reward collection for Game 5 completed.")
