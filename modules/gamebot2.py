...
def run(serial=None):
    ...
    game_duration = 6 * 58  # 6 минут в секундах (360 секунд)
    ...
    while True:
        ...
        if elapsed_time >= game_duration or game_ended:
    ...
    # Если игра не закончилась естественным образом (не найден game_over_template), выполняем цепочку действий
    if elapsed_time >= game_duration and not timer_expired:
        print("6 minutes have passed, executing action chain...")

        # Цепочка действий
        # 1. Нажимаем на первые координаты
        action1_coords = (950, 775)  # Координаты действия 1
        print(f"Tapping action 1 at {action1_coords}")
        tap(device, action1_coords[0], action1_coords[1])

        # 2. Через 0.5 секунды нажимаем на вторые координаты
        time.sleep(0.5)
        action2_coords = (382, 90)  # Координаты действия 2
        print(f"Tapping action 2 at {action2_coords}")
        tap(device, action2_coords[0], action2_coords[1])

        attempt = 0
        while attempt < max_attempts:
            screenshot_path = take_screenshot(device)
            print(f"Attempt {attempt + 1}: Checking if in game...")
            if find_template(screenshot_path, defeat):
                print("In-game interface detected!")
                break
            print("In-game interface not detected yet")
            time.sleep(1)
        ...
        # 3. Через 0.5 секунды нажимаем на третьи координаты
        time.sleep(0.5)
        action3_coords = (1580, 930)  # Координаты действия 3
        print(f"Tapping action 3 at {action3_coords}")
        tap(device, action3_coords[0], action3_coords[1])

        timer_expired = True  # Помечаем, что цепочка выполнена

        # После выполнения цепочки действий ищем новый шаблон gamebot2win.png
        attempt = 0
        while attempt < max_attempts:
            screenshot_path = take_screenshot(device)
            print(f"Attempt {attempt + 1}: Checking for game win interface (gamebot2win.png)...")
            if find_template(screenshot_path, game_win_template, threshold=0.8):
                print("Game win interface (gamebot2win.png) detected!")
                break
            print("Game win interface not detected yet")
            tap(device, confirm_coords[0], confirm_coords[1])
            time.sleep(1)
            attempt += 1
        else:
            print("уебище не задетектилось")
