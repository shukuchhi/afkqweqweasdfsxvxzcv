...
def run(serial=None):
    ...
    interface3_template = os.path.join(base_dir, "templates", "collect2.png")
    print(f"Looking for first interface at: {interface1_template}")
    print(f"Looking for second interface at: {interface2_template}")

    # 1. Нажимаем на координаты
    first_coords = (877, 1000)  # Замени на свои координаты
    ...
    # 5. По скриншоту collect4_2.png нажимаем на кнопку
    attempt = 0
    while attempt < max_attempts:
        screenshot_path = take_screenshot(device)
        print(f"Attempt {attempt + 1}: Checking for second interface...")
        if find_template(screenshot_path, interface3_template):
    ...
    fourth_coords = (101, 1018)  # Замени на свои координаты
    ...
    fifth_coords = (882, 730)  # Замени на свои координаты
    ...
    sixth_coords = (1490, 888)  # Замени на свои координаты
    ...
    ispolzovat = (950, 960)
    print(f"Tapping sixth button at {ispolzovat}")
    tap(device, ispolzovat[0], ispolzovat[1])

    time.sleep(4)

    podtverdit = (1656, 945)
    print(f"Tapping sixth button at {podtverdit}")
    tap(device, podtverdit[0], podtverdit[1])
    
    time.sleep(2)

    strelka = (100,50)
    print(f"Tapping sixth button at {strelka}")
    tap(device, strelka[0], strelka[1])
    ...
    print("Reward collection for Game 4 completed.")
